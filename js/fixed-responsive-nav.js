...
(function () {
  ...
  if ("querySelector" in document && "addEventListener" in window) {
    ...
    var selectActiveMenuItem = function (i) {
      forEach(links, function (i, el) {
        links[i].parentNode.className = links[i].parentNode.className.replace(
          /[\s]{0,}active/,
          ""
        );
      });
      links[i].parentNode.className += links[i].parentNode.className
        ? " active"
        : "active";
    };
      ...
      function () {
        ...
        if (!wasNavigationTapped) {
          forEach(content, function (i, loc) {
            if (
              loc > top &&
              (loc < top + 300 || top + viewport >= bodyheight)
            ) {
